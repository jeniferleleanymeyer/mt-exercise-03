...
def main():
    ...
    with open('scripts/data/logging_00.csv', 'r') as infile_00:
        ...
        for row in reader_00:
            if i > 0:
                valid_perplexity.append([i, row[-1]])
            i += 1
    with open('scripts/data/logging_01.csv', 'r') as infile_01:
        ...
        for row in reader_01:
            if i > 0:
                valid_perplexity[i-1].append(row[-1])
            i += 1
    ...
    with open('scripts/data/sum_table.csv', 'w', encoding='utf-8') as outfile:
        ...
        writer.writerows(valid_perplexity)
    # create plots
    x = []
    y_00 = []
    y_01 = []
    ...
    y_06 = []
    y_09 = []
    for i in range(len(valid_perplexity)-1):
        x.append(valid_perplexity[i][0])
        y_00.append(round(float(valid_perplexity[i][1]), 2))
        y_01.append(round(float(valid_perplexity[i][2]), 2))
        y_03.append(round(float(valid_perplexity[i][3]), 2))
        y_05.append(round(float(valid_perplexity[i][4]), 2))
        y_06.append(round(float(valid_perplexity[i][5]), 2))
        y_09.append(round(float(valid_perplexity[i][6]), 2))
    ...
    plt.title('valid. perplexity of different dropout values')
    plt.xlabel('epoch')
